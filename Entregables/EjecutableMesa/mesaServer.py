...
def agent_portrayal(agent):
    ...
    if (isinstance(agent, Destination)):
        portrayal["Shape"] = "./Entregables/EjecutableMesa/parking.png"
        portrayal["Layer"] = 0
        portrayal["text"] = agent.pos
        portrayal["text_color"] = "black"
    ...
    if (isinstance(agent, Semaforo)):
        ...
        portrayal["Shape"] = "./Entregables/EjecutableMesa/rojo.png" if not agent.state else "./Entregables/EjecutableMesa/verde.png"
        portrayal["Layer"] = 0
        portrayal["w"] = 1
        portrayal["h"] = 1
    ...
    if (isinstance(agent, Mcqueen)):
        # Para su visualizacion correcta, revisar que sea la ruta correcta de las imágenes.
        portrayal["Shape"] = "./Entregables/EjecutableMesa/cuchao.png"
        portrayal["Layer"] = 1
...
def runMesa(numeroAgentes):
    # Setting the width and height of the grid to 0.
    width = 0
    height = 0

    # Reading the file and getting the width and height of the grid.
    with open('./Entregables/EjecutableMesa/2022_base.txt') as baseFile: # Para su funcionamiento correcto, revisar que sea la ruta correcta del archivo a leer.
    ...
    agentes = int(numeroAgentes)
    # time == timpo de estado de semaforo
    model_params = {"N": agentes, "tiempo": 8}
