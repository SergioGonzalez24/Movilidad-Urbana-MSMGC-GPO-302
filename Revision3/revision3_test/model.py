...
class TrafficModel(Model):
    def __init__(self, N, carAutoRegenerate, lightSpan):
        """
        Class initializer (constructor) to create a new Robot agent
        ● Parameters: 
            N: Number of initial cars randomly distributes
            carAutoRegenerate: Number of steps a new car has to wait to appear
            lightSpan: Number of steps the traffic light will remain on each color
        ● Return: None
        """
        self.destCoords = []
        self.carIDs = 0
        self.carAutoRegenerate = carAutoRegenerate 
        self.lightSpan = lightSpan
        
        dataDictionary = json.load(open("./Revision3/revision3_test/mapDictionary.json"))
    ...
    def step(self):
        ...
        # Every carAutoRegenerate steps, creates a new car, assigns it a destination and places it in 
        # any of the corners of the board
        if self.schedule.steps % self.carAutoRegenerate == 0:
            carAgent = Car(self.carIDs, self, choice(self.destCoords))
